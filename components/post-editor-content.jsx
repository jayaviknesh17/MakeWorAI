    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.source === "gemini_ai") {
          toast.success(
            `Content ${type === "generate" ? "generated" : improvementType + "d"} successfully with AI!`
          );
        } else {
          toast.success(
            `Smart content ${type === "generate" ? "generated" : improvementType + "d"} successfully!`
          );
        ...
        // If there's fallback content available, offer to use it
        if (result.fallback && type === "generate") {
          setTimeout(() => {
            toast.info("Using fallback content structure to help you get started", {
              action: {
                label: "Use Template",
                onClick: () => {
                  setValue("content", result.fallback);
                  toast.success("Template content added! You can now customize it.");
                }
              },
              duration: 8000
            });
