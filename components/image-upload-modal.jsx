...
export default function ImageUploadModal({
  ...
  // Apply transformations using Canvas API
  const applyTransformations = async () => {
    ...
    try {
      ...
      if (watchedValues.aspectRatio !== "original") {
        const ratio = ASPECT_RATIOS.find(r => r.value === watchedValues.aspectRatio);
        if (ratio && ratio.width && ratio.height) {
          ...
          targetWidth = watchedValues.customWidth;
          targetHeight = watchedValues.customHeight;
        }
      }
      ...
      // Fill background (white for non-transparent images)
      if (!watchedValues.backgroundRemoved) {
        ctx.fillStyle = '#ffffff';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
      }

      // Draw the image (resized/cropped)
      ...
      if (watchedValues.textOverlay?.trim()) {
        ctx.font = `${watchedValues.textFontSize}px Arial, sans-serif`;
        ctx.fillStyle = watchedValues.textColor;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';

        // Add text stroke for better visibility
        ctx.strokeStyle = watchedValues.textColor === '#ffffff' ? '#000000' : '#ffffff';
        ctx.lineWidth = 2;

        let textX = canvas.width / 2;
        let textY = canvas.height / 2;

        // Adjust position based on selected position
        switch (watchedValues.textPosition) {
          case 'north_west':
        ...
        ctx.strokeText(watchedValues.textOverlay, textX, textY);
        ctx.fillText(watchedValues.textOverlay, textX, textY);
      }

      // Add drop shadow effect if enabled
      if (watchedValues.dropShadow) {
        // Create a new canvas for shadow effect
        ...
        // Convert shadow canvas to data URL
        const shadowDataUrl = shadowCanvas.toDataURL('image/png');
        setTransformedImage(shadowDataUrl);
      } else {
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700">
                    ...
                    <p className="text-sm text-slate-400">
                      Make background transparent (PNG format)
                    </p>
                  </div>
